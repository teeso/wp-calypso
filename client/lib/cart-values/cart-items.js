...
export function cartItemShouldReplaceCart( cartItem, cart ) {
	...
	if ( isDotComPlan( cartItem ) && hasOnlyDomainProductsForDomain( cart, cartItem.meta ) ) {
		return false;
	}
...
/**
 * Does the cart contain only domains and transfers products for the given domain
 *
 * @param {Object} cart - cart as `CartValue` object
 * @param {String} domainName - domain name
 * @return {Boolean} true if there are only domains and transfers products for the given domain
 */
function hasOnlyDomainProductsForDomain( cart, domainName = '' ) {
	if ( isEmpty( domainName ) ) {
		return false;
	}
	return (
		cart &&
		every( [ ...getDomainRegistrations( cart ), ...getDomainTransfers( cart ) ], cartItem => {
			return cartItem.meta === domainName;
		} )
	);
}
